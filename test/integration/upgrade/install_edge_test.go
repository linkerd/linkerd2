...
//////////////////////
//TODO:
// refactor first fn to get long lived binary by combining tests
// refactor combined output in TestHelper.CmdRun(filepath, args...)
// refacto
var (
...
func TestInstallLinkerd(t *testing.T) {
	...
	if err := TestHelper.GetCLIBinary(cliPath, linkerdBaseEdgeVersion); err != nil {
		testutil.AnnotatedFatal(t, "'failed to fetch cli' command failed", err)
	}

	cmd := []string{
		"install",
		"--controller-log-level", "debug",
		// TODO (matei): remove this once flag is deprecated
		"--set", fmt.Sprintf("proxyInit.ignoreInboundPorts=1234\\,5678"),
	}

	// Pipe cmd & args to `linkerd`
	out, moreerr, err := combinedOutput("", cliPath, cmd...)
	if err != nil {
		testutil.AnnotatedFatal(t, "'linkerd install' command failed: %v", err, moreerr)
	}
...
func TestVersionPreInstall(t *testing.T) {
	out, err := TestHelper.LinkerdRun("version")
	if err != nil {
	...
	if !strings.Contains(out, fmt.Sprintf("Server version: %s", linkerdBaseEdgeVersion)) {
		testutil.AnnotatedFatalf(t, "Version command failed", "Version command failed\nexpected: %s, got: %s", linkerdBaseEdgeVersion, out)
	}
}
