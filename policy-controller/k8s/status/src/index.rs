...
impl Controller {
    ...
    /// should be applied to update the status of an xRoute. A patch should
...
impl Index {
    ...
    /// the index reconciles the statuses for all xRoutes on the cluster.
    ///
    ...
    pub async fn run(index: Arc<RwLock<Self>>, reconciliation_period: Duration) {
        ...
        loop {
            ...
            // xRoutes to account for any errors. In either case, we should
            ...
            if let Some(patch) = self.make_route_patch(id, route) {
                match self.updates.try_send(Update {
                    ...
                    Err(error) => {
                        ...
                        tracing::error!(%id.namespace, route = ?id.gkn, %error, "Failed to send {} patch", id.gkn.group.as_ref());
                    }
                }
            }
        }
    ...
    fn index_route(&mut self, id: NamespaceGroupKindName, route: RouteRef) {
        ...
        if let Some(patch) = self.make_route_patch(&id, &route) {
            match self.updates.try_send(Update {
                ...
                Err(error) => {
                    self.metrics.patch_channel_full.inc();
                    tracing::error!(%id.namespace, route = ?id.gkn, %error, "Failed to send {} patch", id.gkn.kind.as_ref());
                }
            }
        }
    }
