...
fn route_attaches_to_server() {
    ...
    assert!(rx
        .borrow_and_update()
        .http_routes
        .contains_key(&inbound::HttpRouteRef::Linkerd("route-foo".to_string())));
    ...
    assert!(rx.has_changed().unwrap());
    assert!(
        rx.borrow().http_routes[&inbound::HttpRouteRef::Linkerd("route-foo".to_string())]
            .authorizations
...
fn routes_created_for_probes() {
    ...
    let probes = update
        .http_routes
        .get(&inbound::HttpRouteRef::Default("probe"))
        .unwrap();
    ...
    assert!(!rx
        .borrow_and_update()
        .http_routes
        .contains_key(&inbound::HttpRouteRef::Default("probes")));
}
