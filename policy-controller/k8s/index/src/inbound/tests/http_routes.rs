...
#[test]
fn probes_from_multiple_containers() {
    let policy = DefaultPolicy::Allow {
        authenticated_only: false,
        cluster_only: true,
    };
    ...
    // Create a pod with two containers which both define probe paths on the
    // same port.
    let containers = [
        k8s::Container {
            liveness_probe: Some(k8s::Probe {
    ...
    let mut pod = mk_pod_with_containers("ns-0", "pod-0", containers);
    pod.labels_mut()
        .insert("app".to_string(), "app-0".to_string());
    test.index.write().apply(pod);
    ...
    let liveness_match_a = HttpRouteMatch {
        path: Some(PathMatch::Exact("/liveness-container-1".to_string())),
        headers: vec![],
        query_params: vec![],
        method: Some(Method::GET),
    };
    let ready_match_a = HttpRouteMatch {
        path: Some(PathMatch::Exact("/ready-container-1".to_string())),
        headers: vec![],
        query_params: vec![],
        method: Some(Method::GET),
    };
    let liveness_match_b = HttpRouteMatch {
        path: Some(PathMatch::Exact("liveness-container-2".to_string())),
        headers: vec![],
        query_params: vec![],
        method: Some(Method::GET),
    };
    let ready_match_b = HttpRouteMatch {
        path: Some(PathMatch::Exact("ready-container-2".to_string())),
        headers: vec![],
        query_params: vec![],
        method: Some(Method::GET),
    };
    ...
    let probes = update.http_routes.get(&RouteRef::Default("probe")).unwrap();
    let probes_rules = probes.rules.first().unwrap();
    println!("probe rules: {:#?}", probes_rules);
    assert!(
        probes_rules.matches.contains(&liveness_match_a),
        "matches: {:#?}",
        probes_rules.matches
    );
    assert!(
        probes_rules.matches.contains(&ready_match_a),
        "matches: {:#?}",
        probes_rules.matches
    );
    assert!(
        probes_rules.matches.contains(&liveness_match_b),
        "matches: {:#?}",
        probes_rules.matches
    );
    assert!(
        probes_rules.matches.contains(&ready_match_b),
        "matches: {:#?}",
        probes_rules.matches
    );
    assert_eq!(probes.authorizations, expected_authorizations);
