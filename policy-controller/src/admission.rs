...
#[instrument]
pub async fn mutate_handler(body: AdmissionReview<DynamicObject>, admission: Admission) -> Result<impl Reply, Infallible> {
    ...
    let servers = api.list(&params).await.unwrap_or_else(|err| {
        ...
        ObjectList{ metadata: Default::default(), items: Default::default() }
    });
    
    // Then construct a AdmissionResponse
    let mut res = AdmissionResponse::from(&req);

    let conflict = req.object.and_then(|obj| {
        obj.data.clone().get_mut("spec").cloned()
    }).and_then(|spec| {
        serde_json::from_value::<ServerSpec>(spec)
        .map_err(|err| { error!(error = ?err.to_string(), "failed to deserialize"); err })
        .ok()
    }).and_then(|spec| {
        for s in servers {
            if s.spec == spec {
