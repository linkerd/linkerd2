...
// These tests are copies of the tests in outbound_api_gateway_http.rs but using the
// Gateway API GrpcRoute kubernetes types instead of HttpRoute.
...
#[tokio::test(flavor = "current_thread")]
async fn service_with_no_grpc_routes() {
    with_temp_ns(|client, ns| async move {
        // Create a service
        ...
        // There should be a default route, but it should be `Detect`
        // and all of its associated routes should be defaults as well
        with_detect_routes(&config, |routes| {
            let route = assert_singleton(routes);
            is_default_http_route(route, &svc, 4191);
        });
    })
    .await;
}
...
#[tokio::test(flavor = "current_thread")]
async fn service_with_grpc_route_without_rules() {
    with_temp_ns(|client, ns| async move {
        // Create a service
        let svc = create_service(&client, &ns, "my-svc", 4191).await;
        ...
        // There should be a default route, but it should be `Detect`
        // and all of its associated routes should be defaults until
        // a `GRPCRoute` is actually created
        with_detect_routes(&config, |routes| {
            let route = assert_singleton(routes);
            is_default_http_route(route, &svc, 4191);
        });

        let _route = create(&client, mk_empty_grpc_route(&ns, "foo-route", &svc, 4191)).await;
        ...
        with_grpc_routes(&config, |routes| {
            let route = assert_singleton(routes);
...
#[tokio::test(flavor = "current_thread")]
async fn service_with_grpc_routes_without_backends() {
    with_temp_ns(|client, ns| async move {
        // Create a service
        let svc = create_service(&client, &ns, "my-svc", 4191).await;
...
#[tokio::test(flavor = "current_thread")]
async fn service_with_grpc_routes_with_backend() {
    with_temp_ns(|client, ns| async move {
        // Create a service
        let svc = create_service(&client, &ns, "my-svc", 4191).await;
        ...
        let route = mk_grpc_route(&ns, "foo-route", &svc, Some(4191)).with_backends(
            Some(&backends),
            None,
            None,
        );
...
#[tokio::test(flavor = "current_thread")]
async fn service_with_grpc_routes_with_cross_namespace_backend() {
    with_temp_ns(|client, ns| async move {
        // Create a service
        let svc = create_service(&client, &ns, "my-svc", 4191).await;
...
#[tokio::test(flavor = "current_thread")]
async fn service_with_grpc_routes_with_invalid_backend() {
    with_temp_ns(|client, ns| async move {
        // Create a service
        let svc = create_service(&client, &ns, "my-svc", 4191).await;
...
// case instead of two.
#[tokio::test(flavor = "current_thread")]
async fn service_with_multiple_grpc_routes() {
    with_temp_ns(|client, ns| async move {
        // Create a service
        let svc = create_service(&client, &ns, "my-svc", 4191).await;
        ...
        // There should be a default route, but it should be `Detect`
        // and all of its associated routes should be defaults until
        // at least one `GRPCRoute` is actually created
        with_detect_routes(&config, |routes| {
            let route = assert_singleton(routes);
            is_default_http_route(route, &svc, 4191);
        });
        ...
        let _a_route = create(
            &client,
            mk_grpc_route(&ns, "a-route", &svc, Some(4191)).build(),
        )
        .await;
        ...
        let _b_route = create(
            &client,
            mk_grpc_route(&ns, "b-route", &svc, Some(4191)).build(),
        )
        .await;
...
async fn route_with_filters() {
    with_temp_ns(|client, ns| async move {
        ...
        let backend_name = "backend";
        let backends = [backend_name];
        let route = mk_grpc_route(&ns, "foo-route", &svc, Some(4191))
        ...
        with_grpc_routes(&config, |routes| {
            ...
            assert_eq!(
                *filters,
                vec![grpc::outbound::grpc_route::Filter {
                    kind: Some(
                        grpc::outbound::grpc_route::filter::Kind::RequestHeaderModifier(
...
#[tokio::test(flavor = "current_thread")]
async fn grpc_route_with_no_port() {
    with_temp_ns(|client, ns| async move {
        // Create a service
        let svc = create_service(&client, &ns, "my-svc", 4191).await;
        ...
        // and all of its associated routes should be defaults until
        // a `GRPCRoute` is actually created
        with_detect_routes(&config_4191, |routes| {
            let route = assert_singleton(routes);
            is_default_http_route(route, &svc, 4191);
        });
        with_detect_routes(&config_9999, |routes| {
            let route = assert_singleton(routes);
            is_default_http_route(route, &svc, 9999);
        });

        let _route = create(&client, mk_grpc_route(&ns, "foo-route", &svc, None).build()).await;
        ...
        with_grpc_routes(&config_4191, |routes| {
            let route = assert_singleton(routes);
        ...
        with_grpc_routes(&config_9999, |routes| {
            let route = assert_singleton(routes);
            assert_route_name_eq(route, "foo-route");
        });
    })
...
async fn producer_route() {
    with_temp_ns(|client, ns| async move {
        ...
        // and all of its associated routes should be defaults until
        // a `GRPCRoute` is actually created
        with_detect_routes(&producer_config, |routes| {
            let route = assert_singleton(routes);
            is_default_http_route(route, &svc, 4191);
        });
        with_detect_routes(&consumer_config, |routes| {
            let route = assert_singleton(routes);
            is_default_http_route(route, &svc, 4191);
        });
        ...
        with_grpc_routes(&producer_config, |routes| {
            let route = assert_singleton(routes);
            assert_route_name_eq(route, "foo-route");
        });

        // The route should be returned in queries from a consumer namespace.
        with_grpc_routes(&consumer_config, |routes| {
            let route = assert_singleton(routes);
            assert_route_name_eq(route, "foo-route");
        });
    })
...
async fn consumer_route() {
    with_temp_ns(|client, ns| async move {
        ...
        with_detect_routes(&other_config, |routes| {
            let route = assert_singleton(routes);
            is_default_http_route(route, &svc, 4191);
        });
        ...
        let _route = create(
            &client,
            mk_grpc_route(&consumer_ns_name, "foo-route", &svc, Some(4191)).build(),
        )
        .await;
        ...
        // The route should NOT be returned in queries from a
        ...
        match policy_api.watch(ns, svc, port).await {
            ...
            Err(error) => {
                tracing::error!(
                    ...
                    "failed to watch outbound policy for port {}",
                    port
                );
...
impl GrpcRouteBuilder {
    fn with_backends(
        ...
        backend_filters: Option<Vec<k8s_gateway_api::GrpcRouteFilter>>,
    ) -> Self {
        ...
        let backend_refs = backends.map(|names| {
            names
                .iter()
                .map(|name| k8s_gateway_api::GrpcRouteBackendRef {
    ...
    fn with_filters(self, filters: Option<Vec<k8s_gateway_api::GrpcRouteFilter>>) -> Self {
        let mut route = self.0;
        route
            .spec
            .rules
...
// assert that the given outbound policy has a proxy
// protocol of "Grpc" and then invoke the given function
// with the config's associated routes.
#[track_caller]
fn with_grpc_routes<F>(config: &grpc::outbound::OutboundPolicy, f: F)
where
    F: Fn(&[grpc::outbound::GrpcRoute]),
{
    let kind = config
        .protocol
        .as_ref()
...
#[track_caller]
fn with_detect_routes<F>(config: &grpc::outbound::OutboundPolicy, f: F)
where
    F: Fn(&[grpc::outbound::HttpRoute]),
{
    ...
    }) = kind
    {
        let http1 = http1
            .as_ref()
            .expect("expected proxy protocol to have an `http1` field");
        let http2 = http2
            .as_ref()
            .expect("expected proxy protocol to have an `http2` field");

        f(&http1.routes);
        f(&http2.routes);
    } else {
        panic!("expected proxy protocol to be Detect; actually got:\n{kind:?}")
    }
}
    ...
    F: Fn(Option<&grpc::outbound::FailureAccrual>),
{
    ...
    match kind {
        grpc::outbound::proxy_protocol::Kind::Grpc(grpc::outbound::proxy_protocol::Grpc {
            failure_accrual,
            ..
        }) => {
            f(failure_accrual.as_ref());
        }
        grpc::outbound::proxy_protocol::Kind::Detect(grpc::outbound::proxy_protocol::Detect {
            opaque: _,
            timeout: _,
            http1,
            http2,
        ...
        _ => panic!("expected proxy protocol to be either Grpc or Detect; actually got:\n{kind:?}"),
    }
}
...
fn route_backends_first_available(
    route: &grpc::outbound::GrpcRoute,
) -> &[grpc::outbound::grpc_route::RouteBackend] {
    ...
    match kind {
        grpc::outbound::grpc_route::distribution::Kind::FirstAvailable(fa) => &fa.backends,
...
fn route_backends_random_available(
    route: &grpc::outbound::GrpcRoute,
) -> &[grpc::outbound::grpc_route::WeightedRouteBackend] {
    let kind = assert_singleton(&route.rules)
        .backends
        .as_ref()
        .expect("Rule must have backends")
        ...
        .expect("Backend must have kind");
    match kind {
        grpc::outbound::grpc_route::distribution::Kind::RandomAvailable(dist) => &dist.backends,
        _ => panic!("Distribution must be RandomAvailable"),
    }
}

#[track_caller]
fn route_name(route: &grpc::outbound::GrpcRoute) -> &str {
...
#[track_caller]
fn assert_backend_has_failure_filter(backend: &grpc::outbound::grpc_route::WeightedRouteBackend) {
    let filter = assert_singleton(&backend.backend.as_ref().unwrap().filters);
    match filter.kind.as_ref().unwrap() {
        grpc::outbound::grpc_route::filter::Kind::FailureInjector(_) => {}
        _ => panic!("backend must have FailureInjector filter"),
    };
}

#[track_caller]
fn is_default_http_route(route: &grpc::outbound::HttpRoute, svc: &k8s::Service, port: u16) {
...
fn assert_backend_matches_service(
    backend: &grpc::outbound::grpc_route::RouteBackend,
    svc: &k8s::Service,
    port: u16,
) {
    let backend = backend.backend.as_ref().unwrap();
...
#[track_caller]
fn assert_route_name_eq(route: &grpc::outbound::GrpcRoute, name: &str) {
    let kind = route.metadata.as_ref().unwrap().kind.as_ref().unwrap();
    match kind {
