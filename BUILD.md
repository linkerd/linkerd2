...
# Updating install templates

Install configuration is stored in `chart/templates/*.yaml` as HELM-templated
YAML. When these files are changed, tests will fail until the files in
`cli/cmd/testdata/install_*.golden` are updated to reflect the change. These
