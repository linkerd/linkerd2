...
- `trigger_mode`: When default to `TRIGGER_MODE_MANUAL`, Tilt will only perform
  manual rebuilds as triggered from the dashboard. See the
...
mode which means it won't trigger rebuilds on your every keystroke. When you are
...
##### Remote Clusters
If the feature you are working on require testing on remote clusters, apply the
following changes to Tilt, so that you don't have to retag the Linkerd2
component images:
...
logs via Tilt's `Share Snapshot` features on the Linkerd
