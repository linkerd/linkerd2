...
# TODO this should be read from the multicluster values.yaml
...
# Ensure the default (source) cluster is initialized and then ensure that an
# additional (target) cluster is created with a load balancer in front of it.
_mc-init: _k3d-init
    {{ just_executable() }} \
        k3d-k8s='{{ k3d-k8s }}' \
        k3d-name='{{ k3d-name }}-target' \
...
mc-test: _mc-init
	go test -test.timeout=10m --failfast --mod=readonly \
        ./test/integration/multicluster/... \
        -integration-tests -linkerd='{{ justfile_directory() / "bin/linkerd" }}' \
        -multicluster-source-context='k3d-{{ k3d-k8s }}' \
        -multicluster-target-context='k3d-{{ k3d-k8s }}-target'
