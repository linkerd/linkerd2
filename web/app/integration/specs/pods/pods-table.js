const assert = require('assert');
    ...
    it('should check the page for proper table structure', () => {
        browser.url('http://127.0.0.1:7777/pods');
        browser.waitUntil(() => {return $('table').isExisting();
          }, 10000, 'expected table to exist');
        http_headers=["Namespace", "Pod","Meshed", "Success Rate", "RPS", "P50 Latency", 
        "P95 Latency", "P99 Latency", "TLS", "Grafana"];
        tcp_headers=["Namespace","Pod","Meshed","Connections", "Read Bytes / sec","Write Bytes / sec", "Grafana"];
