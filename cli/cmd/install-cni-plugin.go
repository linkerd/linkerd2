...
'linkerd install --linkerd-cni-enabled' command.`,
	...
	cmd.PersistentFlags().UintSliceVar(&options.portsToRedirect, "redirect-ports", options.portsToRedirect, "Port to redirect to proxy, if no port is specified then ALL ports are redirected")
