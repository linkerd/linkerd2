...
func makeInstallUpgradeFlags(defaults *l5dcharts.Values) ([]flag.Flag, *pflag.FlagSet, error) {
	...
	flags := []flag.Flag{
		...
		flag.NewStringFlag(installUpgradeFlags, "identity-trust-anchors-file", "",
			...
			func(values *l5dcharts.Values, value string) error {
				if value != "" {
					// the user supplied value is okay here
					//nolint:gosec
	...
	if release {
		// the "control-plane-version" flag exists
		//nolint:gosec
		installUpgradeFlags.MarkHidden("control-plane-version")
	}
	// the following flags exist
	installUpgradeFlags.MarkHidden("control-plane-tracing")           //nolint:gosec
	installUpgradeFlags.MarkHidden("control-plane-tracing-namespace") //nolint:gosec
...
func makeProxyFlags(defaults *l5dcharts.Values) ([]flag.Flag, *pflag.FlagSet) {
	...
	// the following flags exist
	proxyFlags.MarkDeprecated("proxy-memory", "use --proxy-memory-request instead") //nolint:gosec
	proxyFlags.MarkDeprecated("proxy-cpu", "use --proxy-cpu-request instead")       //nolint:gosec
	...
	if release {
		// the following flags exist
		proxyFlags.MarkHidden("proxy-image")        //nolint:gosec
		proxyFlags.MarkHidden("proxy-version")      //nolint:gosec
		proxyFlags.MarkHidden("image-pull-policy")  //nolint:gosec
		proxyFlags.MarkHidden("init-image")         //nolint:gosec
		proxyFlags.MarkHidden("init-image-version") //nolint:gosec
	}
...
func makeInjectFlags(defaults *l5dcharts.Values) ([]flag.Flag, *pflag.FlagSet) {
	...
	// the "disable-identity" flag exists
	injectFlags.MarkHidden("disable-identity") //nolint:gosec
