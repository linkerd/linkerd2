	...
	flags := []flag.Flag{
		...
		flag.NewBoolFlag(installUpgradeFlags, "ha", false, "Enable HA deployment config for the control plane (default false)",
			func(values *l5dcharts.Values, value bool) error {
				...
				if value {
					...
					values.Proxy.Resources.CPU.Request = haValues.Proxy.Resources.CPU.Request
					values.Proxy.Resources.Memory.Request = haValues.Proxy.Resources.Memory.Request
					// NOTE: CPU Limits are not currently set by default HA charts.
					//values.Proxy.Cores = haValues.Proxy.Cores
					//values.Proxy.Resources.CPU.Limit = haValues.Proxy.Resources.CPU.Limit
					values.Proxy.Resources.Memory.Limit = haValues.Proxy.Resources.Memory.Limit
				}
				return nil
			}),
		...
		flag.NewBoolFlag(installUpgradeFlags, "control-plane-tracing", defaults.ControlPlaneTracing,
			"Enables Control Plane Tracing with the defaults", func(values *l5dcharts.Values, value bool) error {
				defaults.ControlPlaneTracing = value
				return nil
			}),
...
func makeInjectFlags(defaults *l5dcharts.Values) ([]flag.Flag, *pflag.FlagSet) {
	...
	flags := []flag.Flag{
		...
		flag.NewStringFlag(injectFlags, "trace-collector", defaults.Proxy.Trace.CollectorSvcAddr,
			"Collector Service address for the proxies to send Trace Data", func(values *l5dcharts.Values, value string) error {
				values.Proxy.Trace.CollectorSvcAddr = value
				return nil
			}),

		flag.NewStringFlag(injectFlags, "trace-collector-svc-account", defaults.Proxy.Trace.CollectorSvcAccount,
			"Service account associated with the Trace collector instance", func(values *l5dcharts.Values, value string) error {
				values.Proxy.Trace.CollectorSvcAccount = value
				return nil
			}),
