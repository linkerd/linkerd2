...
type (
	...
	installIdentityOptions struct {
		...
		trustPEMFile, crtPEMFile, keyPEMFile, identityIssuerMode string
	}
)
...
`
	...
	identityIssuerModeExternal          = "external"
	identityIssuerModeAuto              = "auto"
)
...
func newInstallOptionsWithDefaults() (*installOptions, error) {
	...
	return &installOptions{
		...
		identityOptions: &installIdentityOptions{
			...
			identityIssuerMode: identityIssuerModeAuto,
		},
...
control plane.`,
  ...
  # subcommands.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			shouldExist := options.identityOptions.externalIssuerMode()
...
func (options *installOptions) installOnlyFlagSet() *pflag.FlagSet {
	...
	flags.StringVar(
		&options.identityOptions.identityIssuerMode, "identity-issuer-mode", options.identityOptions.identityIssuerMode,
		"The mode in which identity issuing is being handled (auto by default)",
	)
	flags.StringVar(
...
func errIfLinkerdIdentityissuerSecretMissing(shouldExist bool) error {
	...
	if (secret == nil || kerrors.IsNotFound(err)) && shouldExist {
		return fmt.Errorf("'linkerd-identity-issuer' needs to exist if --identity-issuer-mode=external")
	}
	return nil
}
...
func (idopts *installIdentityOptions) validate() error {
	...
	if idopts.identityIssuerMode != "" && (idopts.identityIssuerMode != identityIssuerModeAuto && idopts.identityIssuerMode != identityIssuerModeExternal) {
		return fmt.Errorf("--identity-issuer-mode must be one of: %s, %s", identityIssuerModeAuto, identityIssuerModeExternal)
	}

	if idopts.externalIssuerMode() {
		if idopts.trustPEMFile == "" {
			return errors.New("a trust anchors file must be specified if --identity-issuer-mode=external")
		}
...
func (idopts *installIdentityOptions) externalIssuerMode() bool {
	return idopts.identityIssuerMode == identityIssuerModeExternal
}
