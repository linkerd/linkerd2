...
func (options *installOptions) validateAndBuild(stage string, flags *pflag.FlagSet) (*l5dcharts.Values, *pb.All, error) {
	...
	// Update add-on fields in values
	if options.config != "" {
		config, err := ioutil.ReadFile(options.config)
		if err != nil {
			return nil, nil, err
		}

		var addonValues *l5dcharts.Values
		if err = yaml.Unmarshal(config, &addonValues); err != nil {
			return nil, nil, err
		}
...
func (options *installOptions) recordableFlagSet() *pflag.FlagSet {
	...
	flags.StringVar(
		...
		"A path to a config file for add-ons",
	)
...
func mergeAddonValues(values, addonValues *l5dcharts.Values) error {
	...
	values.Grafana = addonValues.Grafana
	values.Prometheus = addonValues.Prometheus
