...
func newInstallOptionsWithDefaults() (*installOptions, error) {
	...
	return &installOptions{
		...
		alertmanager:                defaults.AlertManager["enabled"].(bool),
...
func (options *installOptions) recordableFlagSet() *pflag.FlagSet {
	...
	flags.BoolVar(
		&options.alertmanager, "alertmanager", options.alertmanager,
		"Enables Alert Manager for Prometheus with the default configurations",
	)
	flags.StringVar(
...
func (options *installOptions) buildValuesWithoutIdentity(configs *pb.All) (*l5dcharts.Values, error) {
	...
	installValues.AlertManager["enabled"] = options.alertmanager
		...
		for addon, values := range addons {
			...
			case "alertmanager":
				chart = &charts.Chart{
					...
					Dir:       filepath.Join(addOnChartsPath, alertmanagerChartName),
					Namespace: controlPlaneNamespace,
					RawValues: append(rawValues, values...),
...
func parseAddOnValues(values *l5dcharts.Values) (map[string][]byte, error) {
	...
	if values.AlertManager != nil {
		if enabled, ok := values.AlertManager["enabled"].(bool); !ok {
			return nil, fmt.Errorf("invalid value for 'AlertManager.enabled' (should be boolean): %s", values.AlertManager["enabled"])
		} else if enabled {
			data, err := yaml.Marshal(values.AlertManager)
			if err != nil {
				return nil, err
			}

			addonValues[alertmanagerChartName] = data
		}
	}
