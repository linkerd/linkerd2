...
type HTTPRouteLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha3.HTTPRoute, err error)
...
// List lists all HTTPRoutes in the indexer.
func (s *hTTPRouteLister) List(selector labels.Selector) (ret []*v1alpha3.HTTPRoute, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha3.HTTPRoute))
	})
	return ret, err
}
...
type HTTPRouteNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha3.HTTPRoute, error)
...
// List lists all HTTPRoutes in the indexer for a given namespace.
func (s hTTPRouteNamespaceLister) List(selector labels.Selector) (ret []*v1alpha3.HTTPRoute, err error) {
...
// Get retrieves the HTTPRoute from the indexer for a given namespace and name.
func (s hTTPRouteNamespaceLister) Get(name string) (*v1alpha3.HTTPRoute, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha3.Resource("httproute"), name)
	}
	return obj.(*v1alpha3.HTTPRoute), nil
}
