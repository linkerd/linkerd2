...
type handlerFunc func(kubernetes.Interface, *admissionv1beta1.AdmissionRequest) (*admissionv1beta1.AdmissionResponse, error)
...
func NewServer(client kubernetes.Interface, addr, name, controllerNamespace string, rootCA *pkgTls.CA, handler handlerFunc) (*Server, error) {
	...
	s := &Server{server, client, handler, controllerNamespace}
...
func (s *Server) processReq(data []byte) *admissionv1beta1.AdmissionReview {
	...
	admissionResponse, err := s.handler(s.client, admissionReview.Request)
	if err != nil {
