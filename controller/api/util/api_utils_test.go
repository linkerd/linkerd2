...
func TestBuildGatewayRequest(t *testing.T) {
	t.Run("Parses valid time windows", func(t *testing.T) {
		...
		for _, timeWindow := range expectations {
			gatewayRequest, err := BuildGatewayRequest(
				GatewayRequestParams{
					TimeWindow: timeWindow,
				},
			)
			if err != nil {
				t.Fatalf("Unexpected error from BuildGatewayRequest [%s => %s]", timeWindow, err)
			}
			if gatewayRequest.TimeWindow != timeWindow {
				t.Fatalf("Unexpected TimeWindow from BuildGatewayRequest [%s => %s]", timeWindow, gatewayRequest.TimeWindow)
			}
		}
	})
	...
	t.Run("Rejects invalid time windows", func(t *testing.T) {
		...
		for timeWindow, msg := range expectations {
			_, err := BuildGatewayRequest(
				GatewayRequestParams{
					TimeWindow: timeWindow,
				},
			)
			if err == nil {
				t.Fatalf("BuildGatewayRequest(%s) unexpectedly succeeded, should have returned %s", timeWindow, msg)
			}
			if err.Error() != msg {
				t.Fatalf("BuildGatewayRequest(%s) should have returned: %s but got unexpected message: %s", timeWindow, msg, err)
			}
		}
	})
}
