...
func (s *server) Get(dest *pb.GetDestination, stream pb.Destination_GetServer) error {
	...
	if cluster, found := svc.Labels[labels.RemoteDiscoveryLabel]; found {
		...
		if val, found := svc.Labels["multicluster.linkerd.io/exported"]; found {
			if val == "cluster-agnostic" {
				union := NewMulticlusterUnion(
					log.WithFields(logging.Fields{"sub-system": "multicluster-union"}),
					s.k8sAPI,
					s.metadataAPI,
					stream,
					streamEnd,
				...
				if err := union.InitializeAndRunUnion(); err != nil {
					log.Errorf("Failed to initialise union: %v", err)
				}
			}
		}
	}
