...
func (fsw *federatedServiceWatcher) Subscribe(
	...
	if federatedService, ok := fsw.services[id]; ok {
		...
		federatedService.subscribe(port, nodeName, instanceID, messages, resetStream)
		return nil
	} else {
		fsw.RUnlock()
	}
...
func (fsw *federatedServiceWatcher) Unsubscribe(
	...
	if federatedService, ok := fsw.services[id]; ok {
		...
		federatedService.unsubscribe(messages)
	} else {
		fsw.RUnlock()
	}
}
...
func (fs *federatedService) unsubscribe(
	...
	for i, subscriber := range fs.subscribers {
		// Compare channels by their address (they're reference types)
		if &subscriber.messages == &messages {
