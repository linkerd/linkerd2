...
func (s *grpcServer) serviceResourceQuery(ctx context.Context, req *pb.StatSummaryRequest) resourceResult {
	...
	if !req.SkipStats {
		var err error
		dstBasicStats, err = s.getServiceMetrics(ctx, req, req.TimeWindow)
		if err != nil {
			return resourceResult{res: nil, err: err}
		}
	}
...
func (s *grpcServer) getServiceMetrics(ctx context.Context, req *pb.StatSummaryRequest, timeWindow string) (map[dstKey]*pb.BasicStats, error) {
	tsBasicStats := make(map[dstKey]*pb.BasicStats)
	...
	stringToMatch := fmt.Sprintf("%s.%s.svc", service, namespace)
