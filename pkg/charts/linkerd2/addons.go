...
func ParseAddOnValues(values *Values) ([]AddOn, error) {
	...
	if values.Prometheus != nil {
		if enabled, ok := (*values.Prometheus)["enabled"].(bool); !ok {
			return nil, fmt.Errorf("invalid value for 'prometheus.enabled' (should be boolean): %s", (*values.Grafana)["enabled"])
		} else if enabled {
	...
	if values.Tracing != nil {
		if enabled, ok := (*values.Tracing)["enabled"].(bool); !ok {
			return nil, fmt.Errorf("invalid value for 'tracing.enabled' (should be boolean): %s", (*values.Grafana)["enabled"])
		} else if enabled {
