...
func TestMatch(t *testing.T) {
	testCases := []struct {
		...
		{
			name:     "fips build but latest is non-fips",
			expected: "dev-0.1.2",
			actual:   "dev-0.1.2-fips",
			err:      errors.New("is running version 0.1.2-fips but the latest dev version is 0.1.2"),
		},
		{
			name:     "non-fips build but latest is fips",
			expected: "dev-0.1.2-fips",
			actual:   "dev-0.1.2",
			err:      errors.New("is running version 0.1.2 but the latest dev version is 0.1.2-fips"),
		},
		{
