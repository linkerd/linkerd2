			...
			{id: "by default proxy config",
				proxyConfig: &config.Proxy{
					ProxyImage: &config.Image{
						ImageName:  "gcr.io/linkerd-io/proxy:abcde",
						PullPolicy: "Always",
					},
					ProxyInitImage: &config.Image{
						ImageName:  "gcr.io/linkerd-io/proxy-init:abcde",
						PullPolicy: "Always",
					},
...
func TestOverrideProxyConfigs(t *testing.T) {
	t.Run("with annotations but retain other fields", func(t *testing.T) {
		...
		if err := resourceConfig.overrideProxyConfigs(patch); err != nil {
			t.Fatal(err)
		}
