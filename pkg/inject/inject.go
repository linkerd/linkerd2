...
// the kind and name of the workload's roo towner reference
...
func (conf *ResourceConfig) populateMeta(obj runtime.Object) error {
	...
	case *corev1.Pod:
		...
		if conf.rootOwnerRetriever != nil {
			...
			conf.pod.labels[k8s.ProxyRootOwnerLabel] = om.Name
			//kind := tm.GroupVersionKind()
			//conf.pod.labels[k8s.ProxyRootOwnerKindLabel] = fmt.Sprintf("%s/%s:%s", kind.Group, kind.Version, kind.Kind)
			conf.pod.labels[k8s.ProxyRootOwnerKindLabel] = tm.Kind
		}
		conf.pod.labels[k8s.WorkloadNamespaceLabel] = v.Namespace
