...
func (conf *ResourceConfig) GetInboundPorts() string {
	return getPodInboundPorts(conf.pod.spec)
}

func (conf *ResourceConfig) ParseOpaqueOverrides(override string) []util.PortRange {
	return util.ParseContainerOpaquePorts(override, conf.pod.spec.Containers)
}
...
func (conf *ResourceConfig) GetPodPatch(injectProxy bool) ([]byte, error) {
	...
	values, err := GetOverriddenValues(conf.values, conf.getAnnotationOverrides(), conf)
	if err != nil {
...
func (conf *ResourceConfig) GetConfigOverride(annotationKey string) (string, bool) {
...
func (conf *ResourceConfig) CreateOpaquePortsPatch() ([]byte, error) {
	...
	opaquePorts, ok := conf.GetConfigOverride(k8s.ProxyOpaquePortsAnnotation)
	if ok {
