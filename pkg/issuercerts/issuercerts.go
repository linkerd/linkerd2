...
func CheckCertAlgoRequirements(cert *x509.Certificate) error {
	if cert.PublicKeyAlgorithm == x509.ECDSA {
		...
		if k.Params().BitSize != 256 {
			return fmt.Errorf("if using ECDSA, must use P-256 curve for public key, instead P-%d was used", k.Params().BitSize)
		}
		...
		if k.N.BitLen() != 2048 {
			return fmt.Errorf("if using RSA, must use 2048 bit public key, instead %d bit public key was used", k.N.BitLen())
		}
	} else {
		return fmt.Errorf("must use ECDSA or RSA for public key algorithm, instead %s was used", cert.PublicKeyAlgorithm)
	}

	if cert.SignatureAlgorithm != x509.ECDSAWithSHA256 && cert.SignatureAlgorithm != x509.SHA256WithRSA {
		return fmt.Errorf("must be signed by an ECDSA P-256 key or an RSA 2048 bit key, instead %s was used", cert.SignatureAlgorithm)
	}
	return nil
}
