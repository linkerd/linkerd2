...
// WaitUntilDeployReadyWithCtx will block and wait until all given deploys have
// been rolled out and their pods are in a 'ready' status. Before checking
// rollout status, the function will attempt to re-build the Kubernetes
// clientset with a new context. The difference between this and WaitRollout is
// that WaitUntilDeployReady uses CheckPods underneath, instead of relying on
// the 'rollout' command. WaitUntilDeployReady will also retry for a long period
