...
| `Dashboard.Replicas`                                      | Number of replicas of dashboard. Make sure `Dashboard.SessionAffinity` is set to ClientIP to support websockets.                                                                      | `1`                                  |
| `Dashboard.SessionAffinity`                               | Set this to `ClientIP` to make sure that connections from a particular client are passed to the same Pod each time.                                                                   | `ClientIP`                           |
| `Dashboard.SessionAffinityConfig.ClientIP.TimeoutSeconds` | Set the maximum session sticky time by setting `Dashboard.SessionAffinityConfig.clientIP.timeoutSeconds` appropriately. Check Kubernetes documentation for this option.               | `3600`                               |
