...
| `identity.issuer.existingTlsSecret`    | Name of an existing secret in the linkerd install namespace containing "crt.pem" and "key.pem" values to be used instead of the secret created by this chart.                        | ``                                   |
| `identity.issuer.tls.crtPEM`          | Issuer certificate (ECDSA). It must be provided during install if not supplying your own secret.                                                                                                                            ||
| `identity.issuer.tls.keyPEM`          | Key for the issuer certificate (ECDSA). It must be provided during install if not supplying your own secret.                                                                                                                ||
