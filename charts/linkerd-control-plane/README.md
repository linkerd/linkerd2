...
| proxy.inboundDiscoveryCacheUnusedTimeout | string | `""` | Maximum time allowed before an unused inbound discovery result is evicted from the cache, defaults to 90s if unspecified |
...
| proxy.outboundDiscoveryCacheUnusedTimeout | string | `""` | Maximum time allowed before an unused outbound discovery result is evicted from the cache, defaults to 5s if unspecified |
