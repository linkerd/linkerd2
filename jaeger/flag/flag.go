...
type (
	...
	// Helm Values used to render Helm jaeger.  This interface allows us to
	// iterate over flags which have been set and apply their effects to the
	// Values.
	Flag interface {
		Apply(values *jaeger.Values) error
		IsSet() bool
		Name() string
	}
	...
	UintFlag struct {
		...
		apply   func(values *jaeger.Values, value uint) error
	}
	...
	Int64Flag struct {
		...
		Value   int64
		flagSet *pflag.FlagSet
		apply   func(values *jaeger.Values, value int64) error
	}
	...
	StringFlag struct {
		...
		Value   string
		flagSet *pflag.FlagSet
		apply   func(values *jaeger.Values, value string) error
	}
	...
	BoolFlag struct {
		...
		Value   bool
		flagSet *pflag.FlagSet
		apply   func(values *jaeger.Values, value bool) error
	}
	...
	DurationFlag struct {
		...
		Value   time.Duration
		flagSet *pflag.FlagSet
		apply   func(values *jaeger.Values, value time.Duration) error
	}
)
...
func NewUintFlag(flagSet *pflag.FlagSet, name string, defaultValue uint, description string, apply func(values *jaeger.Values, value uint) error) *UintFlag {
	flag := UintFlag{
		name:    name,
		flagSet: flagSet,
		apply:   apply,
...
// when applied.  The flag is attached to the given FlagSet.
func NewInt64Flag(flagSet *pflag.FlagSet, name string, defaultValue int64, description string, apply func(values *jaeger.Values, value int64) error) *Int64Flag {
	flag := Int64Flag{
		name:    name,
		flagSet: flagSet,
		apply:   apply,
...
// when applied.  The flag is attached to the given FlagSet.
func NewStringFlag(flagSet *pflag.FlagSet, name string, defaultValue string, description string, apply func(values *jaeger.Values, value string) error) *StringFlag {
	flag := StringFlag{
		name:    name,
		flagSet: flagSet,
		apply:   apply,
...
// when applied.  The flag is attached to the given FlagSet.
func NewStringSliceFlag(flagSet *pflag.FlagSet, name string, defaultValue []string, description string, apply func(values *jaeger.Values, value []string) error) *StringSliceFlag {
...
// when applied.  The flag is attached to the given FlagSet.
func NewStringFlagP(flagSet *pflag.FlagSet, name string, short string, defaultValue string, description string, apply func(values *jaeger.Values, value string) error) *StringFlag {
	flag := StringFlag{
		name:    name,
		flagSet: flagSet,
		apply:   apply,
...
// when applied.  The flag is attached to the given FlagSet.
func NewBoolFlag(flagSet *pflag.FlagSet, name string, defaultValue bool, description string, apply func(values *jaeger.Values, value bool) error) *BoolFlag {
	flag := BoolFlag{
		name:    name,
		flagSet: flagSet,
		apply:   apply,
...
// when applied.  The flag is attached to the given FlagSet.
func NewDurationFlag(flagSet *pflag.FlagSet, name string, defaultValue time.Duration, description string, apply func(values *jaeger.Values, value time.Duration) error) *DurationFlag {
	flag := DurationFlag{
		name:    name,
		flagSet: flagSet,
		apply:   apply,
...
func (flag *UintFlag) Apply(values *jaeger.Values) error {
...
// Apply executes the stored apply function on the given Values.
func (flag *Int64Flag) Apply(values *jaeger.Values) error {
	return flag.apply(values, flag.Value)
}
...
// Apply executes the stored apply function on the given Values.
func (flag *StringFlag) Apply(values *jaeger.Values) error {
	return flag.apply(values, flag.Value)
}
...
// Apply executes the stored apply function on the given Values.
func (flag *StringSliceFlag) Apply(values *jaeger.Values) error {
	return flag.apply(values, flag.Value)
}
...
// Apply executes the stored apply function on the given Values.
func (flag *BoolFlag) Apply(values *jaeger.Values) error {
	return flag.apply(values, flag.Value)
}
...
// Apply executes the stored apply function on the given Values.
func (flag *DurationFlag) Apply(values *jaeger.Values) error {
	return flag.apply(values, flag.Value)
}
...
func ApplySetFlags(values *jaeger.Values, flags []Flag) error {
