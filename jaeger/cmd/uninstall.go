...
func fetchKubernetesResources(ctx context.Context, k *k8s.KubernetesAPI) ([]resource.Kubernetes, error) {
	...
	namespaces, err := k.CoreV1().Namespaces().List(ctx, metav1.ListOptions{LabelSelector: "linkerd.io/extension=linkerd-jaeger"})
	if err != nil {
		return nil, err
	}
	if len(namespaces.Items) == 0 {
		return nil, errors.New("The linkerd-jaeger extension is not installed")
	}

	resources := make([]resource.Kubernetes, 0)
	// There should never be more than one linkerd-jaeger extension installed
	// but just in case we'll delete them all.
	for _, ns := range namespaces.Items {

			// ClusterRole
	k.RbacV1().ClusterRoles().Get(ctx, )
